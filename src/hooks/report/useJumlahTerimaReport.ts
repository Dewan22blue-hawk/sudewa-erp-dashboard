...
import {
  getBpkbReceiptReport,
  getStnkReceiptReport,
  getSkpdReceiptReport,
  getTnkbReceiptReport,
  JumlahTerimaReportParams,
} from '@/services/report/jumlahTerimaReport.service';

export function useJumlahTerimaReport({
  activeTab,
  page,
  perPage,
  search,
  ...
  const queryResult = useQuery({
    queryKey: ['jumlah-terima-report', activeTab, page, perPage, search, sortBy, sortOrder],
    queryFn: async () => {
      const params: JumlahTerimaReportParams = {
        page,
        per_page: perPage,
      ...
      if (activeTab === 'bpkb') return getBpkbReceiptReport(params);
      if (activeTab === 'stnk') return getStnkReceiptReport(params);
      if (activeTab === 'skpd') return getSkpdReceiptReport(params);
      return getTnkbReceiptReport(params);
    },
