...
import { fetchSalesRefund } from '@/services/refundService';

interface UseRefundJualReturn {
  data: RefundJual[];
...
const defaultPagination: RefundJualPagination = {
  currentPage: 1,
  lastPage: 1,
  perPage: 10,
  total: 0,
...
export function useRefundJual(): UseRefundJualReturn {
  const [data, setData] = useState<RefundJual[]>([]);
  const [pagination, setPagination] = useState<RefundJualPagination>(defaultPagination);
  ...
  const fetchData = useCallback(
    async (page = currentPage, perPage = currentPerPage, search = currentSearch) => {
      ...
      try {
        const result = await fetchSalesRefund({ page, per_page: perPage, search });
        setData(result.data);
        ...
        const message = err instanceof Error ? err.message : 'Gagal mengambil data refund penjualan';
        setError(message);
        setData([]);
