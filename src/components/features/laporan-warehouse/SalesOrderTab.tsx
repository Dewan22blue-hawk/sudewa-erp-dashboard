...
export default function SalesOrderTab({ perPage, dateRange, onActionsChange }: SalesOrderTabProps) {
  ...
  const fetchData = useCallback(async () => {
    setIsLoading(true);
    try {
      const result = await getSalesOrderOutstanding({
        page,
        per_page: perPage,
      ...
      toast.error('Gagal memuat data sales order outstanding');
    } finally {
      setIsLoading(false);
    }
    ...
    () =>
      rows.reduce(
        (accumulator, item) => {
          ...
          accumulator.delivered += item.delivered_qty ?? 0;
          accumulator.remaining += item.remaining_qty;
          return accumulator;
        },
        { order: 0, delivered: 0, remaining: 0 },
      ),
    [rows],
  );
  ...
  const handleDownload = useCallback(() => {
    ...
    lines.push(toCsvLine(['', '', '', '', 'GRAND TOTAL', summary.order, summary.delivered, summary.remaining]));

    const blob = new Blob([lines.join('\n')], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const anchor = document.createElement('a');
    anchor.href = url;
    anchor.download = `warehouse-so-outstanding-page-${page}.csv`;
    document.body.appendChild(anchor);
    anchor.click();
    document.body.removeChild(anchor);
    URL.revokeObjectURL(url);

    toast.success('Data sales order berhasil diunduh');
  }, [page, rows, summary]);
          ...
          <Table>
            <TableHeader className="bg-gray-50">
              <TableRow>
                ...
                <TableHead className="font-semibold whitespace-nowrap">KODE PENJUALAN</TableHead>
                <TableHead className="font-semibold whitespace-nowrap">TGL JUAL</TableHead>
                <TableHead className="font-semibold">CUSTOMER</TableHead>
                <TableHead className="font-semibold">TIPE UNIT</TableHead>
                <TableHead className="text-right font-semibold">QTY JUAL</TableHead>
                <TableHead className="text-right font-semibold">QTY KIRIM</TableHead>
                ...
                <>
                  {pagedRows.map((item, index) => (
                    <TableRow key={`${item.code}-${item.unit_type}-${index}`} className="hover:bg-gray-50">
                      ...
                      <TableCell className="whitespace-nowrap">{item.customer_name || '-'}</TableCell>
                      ...
                      <TableCell className="text-right">{formatNumber(item.delivered_qty ?? 0)}</TableCell>
                  ...
                  <TableRow className="bg-blue-50 font-semibold print-hide-pagination">
                    ...
                    <TableCell className="text-right">{formatNumber(summary.delivered)}</TableCell>
