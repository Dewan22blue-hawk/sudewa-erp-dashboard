    ...
    <DashboardLayout>
      <div className="space-y-10 p-6">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full text-[#111827]" onClick={() => router.push(`/dashboard/${slug}/transaksi/pembelian-unit/${transactionId}/refund`)}>
            <ChevronLeft className="h-5 w-5" />
          </Button>
          <h1 className="text-[22px] font-semibold text-[#111827]">Detail Refund Pembelian</h1>
        </div>
                ...
                {(refund.payments ?? []).length > 0 ? (
                  ...
                  <TableRow>
                    <TableCell colSpan={5} className="h-24 text-center text-[#6B7280]">
                      Belum ada detail refund pembelian.
                    </TableCell>
                  </TableRow>
                )}
              </TableBody>
