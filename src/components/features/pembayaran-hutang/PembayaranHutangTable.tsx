...
export default function PembayaranHutangTable({ data }: Props) {
    const [search, setSearch] = useState("")
    const [itemsPerPage, setItemsPerPage] = useState("25")
    const [currentPage, setCurrentPage] = useState(1)
    const [sortConfig, setSortConfig] = useState<{ key: keyof PembayaranHutang; direction: 'asc' | 'desc' } | null>(null)
    ...
    const filteredData = data.filter(item =>
        item.kodeBayar.toLowerCase().includes(search.toLowerCase()) ||
        item.kodeBeli.toLowerCase().includes(search.toLowerCase()) ||
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                <DropdownMenuItem asChild>
                                                    {slug ? (
                                                        <Link href={`/dashboard/${slug}/finance/data-pembayaran-hutang/${item.id}`}>
                                                            Detail
                                                        </Link>
                                                    ) : (
