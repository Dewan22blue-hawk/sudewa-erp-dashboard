        ...
        <table className="w-full text-sm">
          <thead className="bg-muted uppercase text-xs">
            <tr>
              ...
              <th className="p-2 text-center font-bold text-black border-l border-r" colSpan={4}>
                BANK
              </th>
              <th className="p-2 text-center font-bold text-black border-r" colSpan={2}>
                CASH
              </th>
              <th className="p-0 text-left font-bold text-black min-w-[150px]" rowSpan={2}>
              ...
              <th className="p-2 text-center font-bold text-black" rowSpan={2}>
                Action
              </th>
            </tr>
          ...
          <tbody>
            {sortedData.map((trx) => (
              <tr key={trx.id} className="border-t hover:bg-muted/50 transition-colors">
                ...
                {/* BANK */}
                <td className={`p-2 text-right ${trx.debitUSD ? 'text-green-600 font-medium' : 'text-muted-foreground/50'}`}>{trx.debitUSD ? formatCurrency(trx.debitUSD, 'USD') : '0'}</td>
                <td className={`p-2 text-right ${trx.creditUSD ? 'text-red-600 font-medium' : 'text-muted-foreground/50'}`}>{trx.creditUSD ? formatCurrency(trx.creditUSD, 'USD') : '0'}</td>
                <td className={`p-2 text-right ${trx.debitIDR ? 'text-green-600 font-medium' : 'text-muted-foreground/50'}`}>{trx.debitIDR ? formatCurrency(trx.debitIDR, 'IDR') : '0'}</td>
                <td className={`p-2 text-right border-r ${trx.creditIDR ? 'text-red-600 font-medium' : 'text-muted-foreground/50'}`}>{trx.creditIDR ? formatCurrency(trx.creditIDR, 'IDR') : '0'}</td>

                {/* CASH */}
                <td className={`p-2 text-right ${trx.debitCash ? 'text-green-600 font-medium' : 'text-muted-foreground/50'}`}>{trx.debitCash ? formatCurrency(trx.debitCash, 'IDR') : '0'}</td>
                <td className={`p-2 text-right border-r ${trx.creditCash ? 'text-red-600 font-medium' : 'text-muted-foreground/50'}`}>{trx.creditCash ? formatCurrency(trx.creditCash, 'IDR') : '0'}</td>
