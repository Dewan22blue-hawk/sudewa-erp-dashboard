...
export function PurchaseRefundForm({ purchase, totalPaid, cashOptions, loading = false, submitting = false, onCancel, onSubmit }: Props) {
  const totalPurchase = Number(purchase.unit_transaction_bruto_total ?? purchase.unit_transaction_item_bruto_total ?? 0);
  const refundAmount = useMemo(() => Math.max(0, totalPaid), [totalPaid]);
  const form = useForm<PurchaseRefundFormValues>({
    ...
    <Card className="rounded-[20px] border border-gray-200 bg-white shadow-[0_1px_3px_rgba(15,23,42,0.06)]">
      <CardContent className="p-6 md:p-8">
        <div className="space-y-5">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900">Informasi Pembelian</h2>
            ...
            >
              <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
                <div className="space-y-2">
                  <FormLabel className="text-sm font-medium text-slate-900">Kode Pembelian</FormLabel>
                  <Input value={purchase.code} readOnly className="h-12 rounded-xl border-gray-200 bg-white text-slate-900 shadow-[inset_0_1px_2px_rgba(15,23,42,0.04)]" />
                </div>

                <div className="space-y-2">
                  <FormLabel className="text-sm font-medium text-slate-900">Supplier</FormLabel>
                  <Input value={purchase.person?.name ?? '-'} readOnly className="h-12 rounded-xl border-gray-200 bg-white text-slate-900 shadow-[inset_0_1px_2px_rgba(15,23,42,0.04)]" />
                </div>
                ...
                <div className="space-y-2 md:col-span-2">
                  <FormLabel className="text-sm font-medium text-slate-900">Total Pembelian</FormLabel>
                  <Input value={formatCurrency(totalPurchase)} readOnly className="h-12 rounded-xl border-gray-200 bg-white text-slate-900 shadow-[inset_0_1px_2px_rgba(15,23,42,0.04)]" />
                </div>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="space-y-2 md:col-span-2">
                      <FormLabel className="text-sm font-medium text-slate-900">kas Terima Refund</FormLabel>
