...
import { MoreVertical, Search, ArrowUpDown, ChevronLeft, ChevronRight } from "lucide-react"
...
export default function HutangTable({ data }: Props) {
    ...
    const filteredData = data.filter(item =>
        item.noPembelian.toLowerCase().includes(search.toLowerCase()) ||
                ...
                <table className="w-full text-sm">
                    <thead className="bg-gray-50/50 uppercase text-sm font-semibold text-gray-900 leading-normal">
                        <tr className="border-b border-gray-200">
                            ...
                            <th className="px-4 py-3 text-right">AMOUNT HUTANG</th>
                            ...
                            paginatedData.map((item, i) => (
                                <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-3 text-right">
                                        Rp {item.totalBeli.toLocaleString()}
                                    </td>
                                    <td className="px-4 py-3 text-right">
                                        Rp {item.totalBayar.toLocaleString()}
                                    </td>
                                    <td className="px-4 py-3 text-right text-red-600 font-medium">
                                        Rp {item.amountHutang.toLocaleString()}
                                    </td>
                                        ...
                                        <DropdownMenu>
                                            ...
                                            <DropdownMenuContent align="end">
                                                <DropdownMenuItem asChild>
                                                    <Link href={`/dashboard/${slug}/finance/data-hutang/${item.id}`}>
                                                        Detail
                                                    </Link>
                                                </DropdownMenuItem>
                                            </DropdownMenuContent>
