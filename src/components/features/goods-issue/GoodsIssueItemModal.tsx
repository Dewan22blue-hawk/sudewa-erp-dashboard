...
export function GoodsIssueItemModal({
  ...
  const materialOptions = useMemo(
    () => materials.map((material) => ({ value: String(material.id), label: material.name, subtitle: [material.code, material.type].filter(Boolean).join(' • ') })),
    [materials],
  );
  const form = useForm<GoodsIssueItemFormValues>({
    resolver: zodResolver(goodsIssueItemSchema),
    defaultValues: { materialId: 0, qty: 0, type: 'pcs', price: 0, description: '' },
  });
        ...
        <div className="px-6 py-7">
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Controller control={form.control} name="price" render={({ field }) => <MoneyInput value={field.value} onChangeValue={field.onChange} placeholder="Rp" className="h-10 rounded-[10px] border-slate-200 px-3 text-[15px]" />} />
