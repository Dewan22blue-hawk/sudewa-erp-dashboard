...
import type { RefundJual } from '@/@types/refund-jual.types';
...
export default function RefundJualPage() {
  const { data, pagination, isLoading, isFetching, error, fetchData, setPage, setPerPage, setSearch } = useRefundJual();
  const [searchInput, setSearchInput] = useState('');

  const { sortedData, sortKey, sortOrder, handleSort } = useTableSort<RefundJual>({
    data,
  ...
  return (
    <DashboardLayout>
      <Head>
        <title>Data Refund Penjualan - Wajira Dashboard</title>
      </Head>

      <div className="space-y-6 p-6">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Data Refund Penjualan</h1>
          <p className="text-sm text-gray-500">Kelola data refund penjualan dari API finance.</p>
        </div>
