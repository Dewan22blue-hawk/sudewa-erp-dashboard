...
import { useCreateMaterialTransactionItem, useDeleteMaterialTransactionItem, useMaterialTransaction, useMaterialTransactionItem, useMaterialTransactionItems, useUpdateMaterialTransaction, useUpdateMaterialTransactionItem, useUploadMaterialTransactionInvoice } from '@/hooks/useMaterialTransaction';
...
export default function MaterialReceiptEditPage() {
  ...
  const handleUpdateTransaction = async (values: MaterialTransactionFormValues) => {
    ...
    try {
      ...
      toast.error(error instanceof ApiResponseError ? error.message : 'Gagal memperbarui data penerimaan');
    }
  };
  ...
  if (transactionQuery.isLoading) {
    return (
      <DashboardLayout>
        <div className="rounded-2xl border border-slate-200 bg-white p-10 text-center text-slate-500">Memuat data penerimaan perlengkapan...</div>
      </DashboardLayout>
    );
  }
      ...
      <div className="space-y-6">
        <div className="space-y-1">
          ...
          <p className="text-[16px] text-slate-500">
            No Pemesanan <span className="font-medium text-blue-600">{transactionQuery.data.code}</span>
          </p>
        </div>
          ...
          <Table>
            <TableHeader className="bg-slate-100/90">
              <TableRow className="border-slate-200">
                ...
                <TableHead className="px-4 py-4 text-[14px] font-semibold uppercase text-slate-950">NO PEMBELIAN</TableHead>
