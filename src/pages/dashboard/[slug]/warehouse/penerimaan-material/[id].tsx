...
export default function GoodsReceiptDetailPage() {
  ...
  const billing = useMemo(() => getReceiptBilling(receipt), [receipt]);
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between gap-4">
          ...
          <Button asChild className="h-10 rounded-[10px] bg-[#1f4163] px-5 text-[16px] hover:bg-[#183552]">
            <Link href={`/dashboard/${slug}/warehouse/penerimaan-material/${receipt.id}/edit`}>
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="space-y-6">
            <div className="flex items-center justify-between border-b border-slate-200 pb-6">
              ...
              <Button onClick={() => setOpenInvoice(true)} className="h-10 rounded-[10px] bg-[#1f4163] px-5 text-[16px] hover:bg-[#183552]">
                Lihat Nota
              </Button>
            </div>
        ...
        <Card className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <Table>
            ...
            <TableBody>
              {receipt.goodsTransactionDetails.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={7} className="h-24 text-center text-slate-500">Belum ada detail material.</TableCell>
                </TableRow>
              ) : receipt.goodsTransactionDetails.map((item, index) => (
      ...
      <InvoicePreviewModal open={openInvoice} onOpenChange={setOpenInvoice} invoiceFile={receipt.invoiceFile} />
    </DashboardLayout>
  );
}
