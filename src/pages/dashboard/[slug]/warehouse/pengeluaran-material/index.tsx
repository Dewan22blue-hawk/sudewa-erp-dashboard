...
import {
  useCreateGoodsIssue,
  useCreateGoodsIssueBilling,
  useCreateGoodsIssuePayment,
  useDeleteGoodsIssue,
  useGoodsIssue,
  useGoodsIssues,
  useUploadGoodsIssueInvoice,
...
const readUploadInvoiceError = (error: unknown) => {
  ...
  const apiError = error as ApiError & { details?: Record<string, unknown> | string; response?: { data?: { message?: string; errors?: Record<string, string[]> } } };
  const fieldErrors = apiError.response?.data?.errors;
  const firstFieldError = fieldErrors ? Object.values(fieldErrors)[0]?.[0] : undefined;
  const rawMessage = firstFieldError || apiError.response?.data?.message || (typeof apiError.details === 'string' ? apiError.details : undefined) || apiError.message;
...
export default function GoodsIssueListPage() {
  ...
  const deleteMutation = useDeleteGoodsIssue();
  ...
  const [selectedUpload, setSelectedUpload] = useState<GoodsIssue | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<GoodsIssue | null>(null);
  const [customerSearch, setCustomerSearch] = useState('');

  const paymentDetailQuery = useGoodsIssue(selectedPay?.id);
  ...
  const handleCreate = async (values: GoodsIssueFormValues) => {
    try {
      ...
      toast.success('Data pengeluaran material berhasil dibuat');
      setOpenForm(false);
    } catch (error) {
      const message = error instanceof ApiValidationError || error instanceof ApiResponseError ? error.message : 'Gagal menyimpan data pengeluaran material';
      toast.error(message);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success('Data pengeluaran material berhasil dihapus');
      setDeleteTarget(null);
    } catch (error) {
      const message = error instanceof ApiResponseError ? error.message : 'Gagal menghapus data pengeluaran material';
      toast.error(message);
    }
  };

  const handlePayment = async (values: GoodsIssuePaymentFormValues) => {
    ...
    try {
      ...
      toast.success('Pembayaran pengeluaran material berhasil disimpan');
      setSelectedPay(null);
    } catch (error) {
  ...
  const handleUpload = async (file: File | null) => {
    ...
    try {
      await uploadMutation.mutateAsync({ id: selectedUpload.id, payload: { invoiceFile: file } });
  ...
  return (
    <DashboardLayout>
      ...
      <GoodsReceiptUploadModal open={!!selectedUpload} onOpenChange={(open) => !open && setSelectedUpload(null)} onSubmit={handleUpload} isSubmitting={uploadMutation.isPending} />

      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Hapus data pengeluaran material?</AlertDialogTitle>
