...
let piutangDB: Piutang[] = generatePiutangData()
let paymentDB: Record<string, PiutangPayment[]> = {}

export const PiutangService = {
    getAll: () => piutangDB,

    getById: (id: string) =>
        piutangDB.find(p => p.id === id),
    ...
    addPayment: (id: string, payment: PiutangPayment) => {
        ...
        if (piutang) {
            ...
            piutang.amountPiutang =
                piutang.totalJual - piutang.totalBayar
        }
    },
}
