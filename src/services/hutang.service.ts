...
export const getHutangById = async (
    id: string
): Promise<Hutang | undefined> => {
    return hutangDB.find(h => h.id === id)
}

export const getPaymentsByHutang = async (
    hutangId: string
): Promise<HutangPayment[]> => {
    return paymentDB.filter(p => p.hutangId === hutangId)
}

export const bayarHutang = async (
    hutangId: string,
    data: Omit<HutangPayment, "id" | "hutangId">
) => {
